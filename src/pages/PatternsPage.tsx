...
import { Plus, Search, BookOpen, X, Layers } from 'lucide-react'
...
function PatternCard({ item, usageCount, allGlass, onClick, onPhotoClick }: { item: Pattern; usageCount: number; allGlass: GlassItem[]; onClick: () => void; onPhotoClick?: (images: string[], index: number) => void }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md hover:border-teal-200 dark:hover:border-teal-700 transition-all cursor-pointer" onClick={onClick}>
      ...
      >
        {item.photos[0] ? (
          <img src={item.photos[0]} alt={item.name} className={`w-full h-full object-cover ${allImages.length > 0 && onPhotoClick ? 'cursor-zoom-in' : ''}`} />
        ) : item.lineDrawings[0] ? (
          <img src={item.lineDrawings[0]} alt={item.name} className={`w-full h-full object-cover ${allImages.length > 0 && onPhotoClick ? 'cursor-zoom-in' : ''}`} />
        ) : (
          ...
          {filtered.map(item => (
            <div key={item.id} className="relative group">
              <PatternCard
                ...
                onPhotoClick={(imgs, i) => { setLightboxImages(imgs); setLightboxIndex(i) }}
              />
